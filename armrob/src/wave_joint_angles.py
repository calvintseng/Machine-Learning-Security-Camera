...
# Code adapted from original file: manual_joint_angles.py
...
def wave_joint_angles(): 
    ...
    while not rospy.is_shutdown(): 
        if scan_direction == 'CCW':
            while cmds[4] < np.max(rotlim_45)-np.radians(25):
            #    time.sleep(0.1)
                cmds[4] += np.radians(1)
                ...
                rospy.loginfo('Moving to {}'.format(cmds))
            #time.sleep(3)
            scan_direction = 'CW'
            
        elif scan_direction == 'CW':
            while cmds[4] > np.min(rotlim_45)+np.radians(5):
            #    time.sleep(0.1)
                cmds[4] -= np.radians(1)
                joint_angles_desired_msg.position = cmds
                joint_angles_desired_msg.header.stamp = rospy.Time.now()
